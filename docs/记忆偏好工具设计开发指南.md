...
| 工具名称 | 模块路径 | 核心职责 | 关键入参 | 主要输出 | 测试覆盖 |
| --- | --- | --- | --- | --- | --- |
| 记忆访问工具 | `app/providers/tools/memory_access_tool.py` | 统一封装 mem0 读写，支持写入、检索、更新、删除及历史记录查询，抽象 `update_mode`（append/overwrite） | `action`, `messages`, `query`, `filters`, `memory_id`, `update_mode`, `run_id`, `agent_id` | `{ "operation": action, "status": "success|fallback|disabled", "payload": dict/list }` | `tests/test_memory_preference_tools.py::test_memory_access_*`、`tests/test_memory_service.py` 针对服务层与工具协同回归 |
| 偏好标签抽取工具 | `app/providers/tools/preference_tag_tool.py` | 聚合会话与记忆片段，生成用户画像标签并可批量写入画像服务 | `user_id`, `trip_id`, `messages`, `limit`, `dry_run`, `context_window` | `{ "candidates": [tag], "applied": [tag], "metadata": {...} }` | `tests/test_memory_preference_tools.py::test_preference_tag_*` 验证 dry_run/写库/容错路径 |
| 偏好沉淀建议工具 | `app/providers/tools/preference_suggestion_tool.py` | 基于反馈与偏好生成下次出行建议并追加写入记忆层 | `user_id`, `trip_id`, `feedback_items`, `profile_tags`, `memory_refs`, `run_id` | `{ "suggestions": [...], "generated_at": ts, "version": run_id }` | `tests/test_memory_preference_tools.py::test_preference_suggestion_*` 覆盖成功、降级、记忆写入 |
| 行程上下文同步工具 | `app/providers/tools/trip_context_sync_tool.py` | 阶段切换时同步关键事实到 mem0，默认使用 `overwrite` 保持快照最新 | `user_id`, `trip_id`, `stage_from`, `stage_to`, `facts`, `tool_outputs`, `run_id` | `{ "synced": true, "memory_id": str, "status": ... }` | `tests/test_memory_preference_tools.py::test_trip_context_sync_*` 校验写入模式、禁用记忆等情形 |

> 若新增工具，请在上表补充需求与测试信息，保证智能体团队可追溯工具职责与回归范围。
